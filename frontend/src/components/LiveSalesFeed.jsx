...
import { formatMoneyLocalized, formatTimeLocalized, PAYMENT_LABELS } from '../lib/format.js';
          ...
          {feed.map((ev) => (
            <div key={ev._key} className="live-feed-row">
              ...
              <span className="badge" style={{ marginLeft: 8 }}>
                {PAYMENT_LABELS[ev.paymentMethod] ? t(PAYMENT_LABELS[ev.paymentMethod]) : ev.paymentMethod}
              </span>
              <span style={{ flex: 1, marginLeft: 8, color: '#16a34a', fontWeight: 600 }}>
                + {formatMoneyLocalized(ev.totalUzs, 'UZS', lang)}
              </span>
              <span className="faint mono" style={{ fontSize: 11 }}>
                {formatTimeLocalized(ev.at, lang)}
              </span>
            </div>
          ))}
        </div>
